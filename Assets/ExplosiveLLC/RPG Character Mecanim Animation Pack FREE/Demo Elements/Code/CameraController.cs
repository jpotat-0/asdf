		...
		*/
		...
		private void CameraFollow()
		{
			transform.position = new Vector3(
				(cameraTarget.transform.position.x + offset.x * distance),
				(cameraTarget.transform.position.y + offset.y * height),
				(cameraTarget.transform.position.z + offset.z * height)
				);
			/*
			if (Keyboard.current.eKey.isPressed)
			{
				transform.Rotate(Vector3.up, 5.0f * smoothing);
			}
